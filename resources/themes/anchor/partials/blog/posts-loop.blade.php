...
@foreach ($posts as $post)
    <article id="post-{{ $post->id }}"
        class="overflow-hidden relative col-span-2 p-4 bg-white rounded-2xl border cursor-pointer border-zinc-200 border-zinc-100 dark:bg-black sm:col-span-1"
        ...
        <div class="px-1 py-1">
            <div class="flex gap-x-4 items-center my-3 text-xs">
                ...
                <a href="/blog/{{ $post->category->slug }}"
                    class="relative z-10 px-3 py-1.5 font-medium text-zinc-600 bg-zinc-50 rounded-full hover:bg-zinc-100">{{ $post->category->name }}</a>
            </div>
            ...
            <p class="mt-5 text-sm leading-6 text-zinc-600 line-clamp-3">{{ substr(strip_tags($post->body), 0, 110) }}
