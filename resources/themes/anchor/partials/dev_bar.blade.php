            ...
            <div class="w-full h-full bg-gradient-to-r from-blue-500 via-blue-500 to-purple-600 border-t border-blue-500" :class="{ 'overflow-hidden': open }">
                <div class="flex justify-between w-full h-full">
                    ...
                    <div class="flex items-center h-full">
                        ...
                        <div @click="open=true; url='/docs'; active='docs';" class="flex justify-center items-center px-3 h-full text-xs leading-none cursor-pointer text-zinc-100 hover:bg-red-600" :class="{ 'bg-red-600': active == 'docs' && open, 'bg-transparent': active != 'docs' }">
                            Documentation
                        </div>
                        @if(!auth()->guest() && auth()->user()->can('browse_admin'))
                            <div @click="open=true; url='/admin'; active='admin';" class="flex justify-center items-center h-full text-xs leading-none text-blue-100 cursor-pointer hover:bg-red-600" :class="{ 'px-3': !open, 'px-5': open, 'bg-red-600': active == 'admin' && open, 'bg-red-500': active != 'admin' }">
                                Admin
                            </div>
                        @endif
                    </div>
                    <div x-show="open" @click="open=false" class="flex justify-center items-center px-2 w-auto h-full text-white border-l opacity-75 cursor-pointer border-white/10 hover:bg-red-600 hover:bg-opacity-10 hover:opacity-100">
