...
<div x-data="{ dropdownOpen: false }" class="relative w-full" x-cloak>
    <button @click="dropdownOpen=!dropdownOpen"
        ...
        <span class="flex min-w-0 items-center gap-3">
            ...
            <span class="min-w-0">
                <span
                    class="block truncate font-semibold text-zinc-900 dark:text-zinc-100">{{ Auth::user()->name }}</span>
                <span class="block truncate text-xs text-zinc-500 dark:text-zinc-400">{{ Auth::user()->email }}</span>
            </span>
        </span>
        <svg :class="{ 'rotate-180': dropdownOpen }" class="h-4 w-4 flex-shrink-0 transition duration-200"
    ...
    <div wire:ignore x-show="dropdownOpen" @click.away="dropdownOpen=false"
        x-transition:enter="transition ease-out duration-150" x-transition:enter-start="transform opacity-0 scale-95"
        ...
        <div
            class="overflow-hidden rounded-3xl border border-zinc-200/80 bg-white shadow-[0_20px_60px_rgba(15,23,42,0.12)] dark:border-zinc-800 dark:bg-zinc-950">
            <div class="border-b border-zinc-100 px-4 py-3 dark:border-zinc-800">
                <p class="text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500 dark:text-zinc-400">Signed in
                    as</p>
                <p class="mt-1 truncate text-sm font-semibold text-zinc-900 dark:text-zinc-100">
                    {{ auth()->user()->email }}</p>
            </div>
            ...
            <div class="space-y-1 p-2">
                ...
                <x-app.sidebar-link :hideUntilGroupHover="false" href="{{ route('settings.profile') }}"
                    icon="phosphor-gear-duotone" active="false">
                    Settings
                </x-app.sidebar-link>
                @if (auth()->user()->can('panel.access'))
