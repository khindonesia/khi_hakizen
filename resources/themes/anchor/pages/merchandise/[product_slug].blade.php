...
@php
    ...
    $galleryItems = $product
        ? $product->images->sortBy('sort_order')->map(fn($image) => $resolveImageUrl($image->image_url))->values()
        : collect();
        ...
        if ($urlVariantQuery) {
            ...
            if (!$selectedVariant) {
                $selectedVariant = $product->variants->first(
                    fn($v) => strtolower($v->sku) === strtolower($urlVariantQuery),
                );
            }
            // Check if it matches Variant Name (case-insensitive)
            if (!$selectedVariant) {
                $selectedVariant = $product->variants->first(
                    fn($v) => strtolower($v->name) === strtolower($urlVariantQuery),
                );
            }
        }
        if (!$selectedVariant) {
            $selectedVariant =
                ...
                ($product->variants->firstWhere('status', 'active') ?? $product->variants->first());
        }
    }

    $selectedVariantPrice = $selectedVariant?->price ?? 0;
    $selectedVariantStock = $selectedVariant?->stock_quantity ?? 0;
    $selectedVariantStatus = $selectedVariant && $selectedVariant->isInStock() ? 'In stock' : 'Out of stock';
        ...
        <div class="relative min-h-screen py-12 md:py-16">
            <x-container>
                ...
                <a href="{{ route('merchandise') }}" wire:navigate
                    class="mb-8 inline-flex items-center gap-2 text-sm font-medium text-zinc-500 hover:text-red-700 transition-colors">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
                    ...
                    <div class="lg:col-span-6">
                        <div
                            class="relative overflow-hidden rounded-[28px] border border-zinc-200/80 bg-white shadow-sm">
                            <img src="{{ $galleryItems->first() ?? $fallbackImageUrl }}" alt="{{ $product->name }}"
                                width="800" height="800" fetchpriority="high" decoding="sync"
                        ...
                        @if ($galleryItems->count() > 1)
                            <div class="mt-4 grid grid-cols-4 gap-2">
                                @foreach ($galleryItems->skip(1) as $thumb)
                                    <button type="button"
                                        class="relative overflow-hidden rounded-2xl border border-zinc-200/80 transition-all hover:border-red-300"
                                        onclick="this.parentElement.parentElement.querySelector('img').src='{{ $thumb }}'">
                                        <img src="{{ $thumb }}" alt="Thumbnail" width="200" height="200"
                                            loading="lazy" decoding="async" class="w-full h-20 object-cover">
                                    </button>
                                @endforeach
                            </div>
                        @endif
                    ...
                    <aside class="lg:col-span-6 lg:sticky lg:top-[100px]">
                        <div class="stitch-panel flex flex-col gap-6 p-6 md:p-8" x-data="{
                            ...
                            <div class="flex items-baseline gap-4">
                                <span class="text-2xl font-semibold text-red-700">Rp <span
                                        x-text="priceLabel">{{ number_format($selectedVariantPrice, 0, ',', '.') }}</span></span>
                            ...
                            @if ($product->variants->count() > 0)
                                <div class="space-y-3">
                                    ...
                                    <select x-model.number="selectedVariantId"
                                        class="w-full rounded-2xl border border-zinc-200 bg-white py-2.5 px-3.5 focus:border-red-400 focus:outline-none focus:ring-4 focus:ring-red-100">
                            ...
                            @if ($product->productAttributes->count() > 0)
                                <div class="space-y-4">
                                    @foreach ($product->productAttributes->groupBy('attribute_id') as $group)
                                        ...
                                        <div>
                                            <label
                                                class="mb-1 block text-sm font-medium text-zinc-600">{{ $attr->name }}</label>
                                    ...
                                    @if (auth()->user()->hasRole('Member'))
                                        <div class="flex flex-col gap-3">
                                            <div class="flex items-center gap-3">
                                                <div
                                                    class="flex items-center overflow-hidden rounded-full border border-zinc-200 bg-white shadow-sm">
                                                    <button type="button" x-on:click="decreaseQuantity()"
                                                    ...
                                                    <input type="number" min="1" :max="maxQuantity"
                                                        inputmode="numeric" x-model.number="quantity"
                                                        class="w-14 border-0 bg-transparent p-0 text-center text-sm font-semibold text-zinc-900 focus:outline-none focus:ring-0">
                                                    <button type="button" x-on:click="increaseQuantity()"
                                                        class="flex items-center justify-center px-3 py-2 text-zinc-700 transition-colors hover:bg-red-50 hover:text-red-700"
                                                        :disabled="loading">
                                                ...
                                                <p class="text-sm font-semibold {{ $initialStockColor }}"
                                                    :class="selectedVariant && selectedVariant.stock_quantity > 0 ? (
                                                        selectedVariant.stock_quantity <= 5 ? 'text-amber-600' :
                                            ...
                                            <button type="button" x-on:click="addToCart()"
                                            ...
                                            <p x-show="message" x-transition class="text-sm font-medium mt-1 text-center"
                                                x-text="message"
                                                :class="messageType === 'success' ? 'text-emerald-600' : 'text-rose-600'"
                                                x-cloak></p>
                                        </div>
                                    @else
                                        <!-- User is authenticated but does not have 'member' role -->
                                        <a href="{{ route('settings.profile') }}" wire:navigate
                                            class="w-full rounded-full bg-amber-600 hover:bg-amber-700 text-center font-semibold text-white transition-all py-3 flex items-center justify-center gap-1.5 shadow-sm">
                                            <span class="material-symbols-outlined text-[18px]">verified_user</span>
                                            Verify KYC to Become Member
                                        </a>
                                    @endif
                                @else
                                    <a href="{{ route('join') }}"
                                        class="w-full rounded-full bg-red-600 py-3 text-center font-semibold text-white transition-all hover:bg-red-500">
                                        ...
                                        <button type="button"
                                            ...
                                            <div class="flex items-center gap-2.5">
                                                ...
                                                <span class="material-symbols-outlined text-[16px] text-emerald-600"
                                                    x-show="copied" x-cloak>check</span>
    ...
    @else
        <div class="flex min-h-screen items-center justify-center py-12 md:py-16">
            <div class="text-center">
                ...
                <a href="{{ route('merchandise') }}" wire:navigate
                    class="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-red-700 hover:underline">
                    <span class="material-symbols-outlined text-[18px]">arrow_back</span>
                    Back to Merchandise
                </a>
            </div>
