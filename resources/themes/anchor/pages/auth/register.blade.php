...
<x-layouts.marketing :seo="['title' => config('devdojo.auth.language.register.page_title')]">
        ...
        <div class="w-full max-w-md bg-white dark:bg-zinc-900 rounded-2xl shadow-xl border border-slate-100 dark:border-zinc-800 p-8 md:p-10 relative z-10 transition duration-300">
            ...
            @volt('auth.register')
                <x-auth::elements.container>
                    <x-auth::elements.heading :text="($language->register->headline ?? 'Create Account')" :description="($language->register->subheadline ?? 'No Description')" :show_subheadline="($language->register->show_subheadline ?? false)" />
                    ...
                    <form wire:submit="register" class="space-y-5">
                        @if($showNameField)
                            <x-auth::elements.input :label="config('devdojo.auth.language.register.name')" type="text" wire:model="name" id="name" autofocus="true" required />
                        @endif
                        ...
                        <x-auth::elements.button data-auth="submit-button" rounded="full" submit="true">
                            {{ config('devdojo.auth.language.register.button') }}
                        </x-auth::elements.button>
                    </form>
                    @endif

                    <div class="mt-4 space-x-0.5 text-sm leading-5 text-center text-slate-600 dark:text-slate-400">
                        <span>{{ config('devdojo.auth.language.register.already_have_an_account') }}</span>
